...
const LogViewer: React.FC<LogViewerProps> = ({ logs }) => {
    ...
    const getLogStyle = (level: LogLevel) => {
        switch(level) {
            ...
            case LogLevel.INFO: default: return 'text-gray-400';
        }
    };
    
    const getLogIcon = (level: LogLevel) => {
        switch(level) {
            case LogLevel.ERROR: return <XCircleIcon className="w-4 h-4 text-red-500 flex-shrink-0" />;
            case LogLevel.SUCCESS: return <CheckCircleIcon className="w-4 h-4 text-green-500 flex-shrink-0" />;
            case LogLevel.INFO: default: return <InfoIcon className="w-4 h-4 text-gray-500 flex-shrink-0" />;
        }
    }
